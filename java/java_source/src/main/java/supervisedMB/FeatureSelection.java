...
public class FeatureSelection {
    static void applyBLAST(double portion, double totalMatches, List<Comparison> comparisons, PrintWriter out, String info) {
    ...
    static void applyReciprocalCNP(double totalMatches, float k, List<Comparison> comparisons, PrintWriter out, String info) {
        long startTime = System.nanoTime();
    ...
    public static void main(String[] args) throws IOException {
        ...
        PrintWriter out = new PrintWriter(new File("/home/app/results/feature_selection_java.csv"));
        out.println("dataset;train_size;conf_id;run;init_time;algorithm;matches;comparisons;recall;precision;F1;RT");
            ...
            if (dnames.contains(dname)) {
                ...
                for (int conf_id = 1; conf_id <= num_feature_sets; conf_id++) {
                    ...
                    for (int run = 0; run < num_runs; run++) {
                        ...
                        String info = dname + ";" + train_size + ";" + conf_id + ";" + run + ";" + elapsedTime + ";";

                        applyBLAST(0.35, initialMatches, comparisons, out, info);
                        applyReciprocalCNP(initialMatches, (float) Math.max(1, blockSizes / ((float) noOfEntities)), comparisons, out, info);
                    }
                }
            }
            out.close();
