...
public class Table4 {
    ...
    static void applyCNP(double totalMatches, float k, List<Comparison> comparisons, PrintWriter out, String info, double initTime) {
        long startTime = System.nanoTime();
        final Map<String, Queue<Comparison>> pairsPerEntity1 = new HashMap<>();
        final Map<String, Queue<Comparison>> pairsPerEntity2 = new HashMap<>();
        for (Comparison c : comparisons) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        PrintWriter out = new PrintWriter(new File("/home/app/results/table4.csv"));
        ...
        for (String dname : stats.keySet()) {
            if (dnames.contains(dname)) {
                for (int run = 0; run < num_runs; run++) {
                    ...
                    applyCNP(data.initialMatches, (float) Math.max(1, blockSizes / ((float) noOfEntities)), data.comparisons, out, info, data.runtime);


                    train_size = 50;
                    data = loadData(dname, train_size, blast_feat_id, run);
                    pc = data.retainedMatches / (double) data.initialMatches;
                    pq = data.retainedMatches / (double) data.comparisons.size();
                    f1 = 2 * pc * pq / (pc + pq);
