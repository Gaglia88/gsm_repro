...
object TokenBlocking {
  ...
  def createBlocks(profiles: RDD[Profile], separatorIDs: Array[Long] = Array.emptyLongArray, keysToExclude: Iterable[String] = Nil, removeStopWords: Boolean = false,
    ...
    def createCombiner(e: Long): List[Long] = e :: Nil

    def addElement(l: List[Long], e: Long): List[Long] = e :: l

    def mergeCombiners(l1: List[Long], l2: List[Long]): List[Long] = l1 ::: l2

    val profilePerKey = tokensPerProfile.flatMap(x => x._2.map(y => (y, x._1))).combineByKey(createCombiner, addElement, mergeCombiners).filter(_._2.size > 1)
    ...
    val profilePerKey = a.combineByKey(createCombiner, addElement, mergeCombiners).filter(_._2.size > 1)
    */
