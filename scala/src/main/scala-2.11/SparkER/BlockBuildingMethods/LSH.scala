...
object LSH {
  ...
  def clusterSimilarAttributes2StepsDebug(profiles: RDD[Profile], numHashes: Int, targetThreshold: Double,
                                          separatorIDs: Array[Long], startBlockingThreshold: Double = 1.0,
                                          decThreshold: Double = 0.05, maxFactor: Double = 1.0, numBands: Int = -1,
                                          keysToExclude: Iterable[String] = Nil, computeEntropy: Boolean = false,
                                          separator: String = Settings.SOURCE_NAME_SEPARATOR): (RDD[BlockAbstract], List[KeysCluster]) = {

    @transient lazy val log = org.apache.log4j.LogManager.getRootLogger
    var threshold = startBlockingThreshold
    val dec = decThreshold
    val minBlocks = 1


    var blocks: RDD[BlockAbstract] = null

    do {
      blocks = createBlocks(profiles, 128, threshold, separatorIDs = separatorIDs)
      threshold -= dec
    } while (blocks.count() < minBlocks && threshold > 0)

    log.info("SPARKER - LSH Blocking threshold " + (threshold + dec))

    val profilesBlockedId = profiles.context.broadcast(blocks.flatMap(_.getAllProfiles).distinct().collect())

    log.info("SPARKER - Numero di profili mantenuti " + profilesBlockedId.value.length)

    val profilesBlocked = profiles.filter(p => profilesBlockedId.value.contains(p.id))

    val clusters = LSH.clusterSimilarAttributes(profilesBlocked, numHashes, targetThreshold, maxFactor, numBands, keysToExclude, computeEntropy, separator)

    profilesBlockedId.unpersist()


    (blocks, clusters)
  }

  def clusterSimilarAttributes2Steps(profiles: RDD[Profile], numHashes: Int, targetThreshold: Double,
                                     separatorIDs: Array[Long], startBlockingThreshold: Double = 1.0,
                                     decThreshold: Double = 0.05, maxFactor: Double = 1.0, numBands: Int = -1,
                                     keysToExclude: Iterable[String] = Nil, computeEntropy: Boolean = false,
                                     separator: String = Settings.SOURCE_NAME_SEPARATOR): List[KeysCluster] = {
  ...
  def clusterSimilarAttributesSerial(profiles: List[Profile], numHashes: Int, targetThreshold: Double, maxFactor: Double = 1.0, numBands: Int = -1, keysToExclude: Iterable[String] = Nil, computeEntropy: Boolean = false, separator: String = Settings.SOURCE_NAME_SEPARATOR): List[KeysCluster] = {
    @transient lazy val log = org.apache.log4j.LogManager.getRootLogger
    ...
    /**
      ...
      **/
    val attributesToken: List[(Attr, String)] = profiles.flatMap {
      profile =>
    ...
    /**
      ...
      **/
    val attributesPerToken: List[(Int, Iterable[Attr])] = attributesToken.map(_.swap).groupBy(_._1).map(x => (x._1, x._2.map(_._2))).zipWithIndex.map(x => (x._2, x._1._2)).toList

    /**
      * Genera gli hash per ogni token e li invia in broadcast
      **/
    val hashes2 = attributesPerToken.map { case (tokenID, dataprofiles) =>
    ...
    /**
      ...
      **/
    val tokensPerAttribute: List[(Attr, Set[Int])] = attributesPerToken.flatMap { case (tokenID, attributes) =>
      attributes.map(p => (p, tokenID))
    }.groupBy(_._1).map(x => (x._1, x._2.map(_._2).toSet)).toList
    ...
    /**
      ...
      **/
    val attributeWithSignature: List[(Attr, Array[Int])] = tokensPerAttribute.map { case (attribute, tokens) =>
    ...
    val attributesPerBucket = buckets.flatMap(x => x._2.map((_, x._1))).groupBy(_._1).map(x => (x._1, x._2.map(_._2).toSet)).filter(x => x._2.size > 1 && x._2.size < 101).values.toList.distinct

    val attributeSignatures = attributeWithSignature.toMap
    ...
    val edges = partialClusters.flatMap { clusterElements =>
      ...
      }.map(x => (x.head, (x.last, calcSimilarity(attributeSignatures(x.head), attributeSignatures(x.last)))))
    }
    ...
    val edgesPerKey =
      ...
      ).groupBy(x => x._1).map(x => (x._1, x._2.map(_._2).toSet))
    ...
    val vertices = topEdges.keys.toList.union(topEdges.toList.flatMap(_._2)).distinct

    vertices.foreach { v =>
      graph.addVertex(v)
    }

    topEdges.foreach { case (from, to) =>
      to.foreach { n =>
    ...
    }.toList ::: KeysCluster(defaultClusterID, nonClusteredAttributes.map(k => k.sourceName + separator + k.attribute)) :: Nil

  }

  def clusterSimilarAttributes(profiles: RDD[Profile], numHashes: Int, targetThreshold: Double, maxFactor: Double = 1.0, numBands: Int = -1, keysToExclude: Iterable[String] = Nil, computeEntropy: Boolean = false, separator: String = Settings.SOURCE_NAME_SEPARATOR): List[KeysCluster] = {
    @transient lazy val log = org.apache.log4j.LogManager.getRootLogger
    ...
    /**
      * Genera gli hash per ogni token e li invia in broadcast
      **/
    val hashes2 = sc.broadcast(attributesPerToken.map {
      case (tokenID, dataprofiles) =>
        val hashes = getHashes2(tokenID, numHashes)
        (tokenID, hashes)
    }.collectAsMap())
    ...
    /**
      * Raccoglie per ogni attributo gli id dei token che contiene, si ottiene quindi (attributo, [elenco token])
      **/
    val tokensPerAttribute: RDD[(Attr, Set[Int])] = attributesPerToken.flatMap {
    ...
    /**
      * Genera la signature per ogni attributo
      **/
    val attributeWithSignature: RDD[(Attr, Array[Int])] = tokensPerAttribute.map {
    ...
    val buckets = attributeWithSignature.map {
    ...
    /**
      ...
      * Produces a list of (attr1, (attr2, JaccardSimilarity(attr1, attr2))
      **/
    val edges = partialClusters.flatMap {
      clusterElements =>
        ...
        /**
          ...
          * attributes that belongs from the second.
          **/
        clusterElements.toList.combinations(2).filter {
          x =>
            x.head.sourceName != x.last.sourceName
    ...
    /*edges.collect().foreach(e =>
      log.info("TESTDOM "+e._1.sourceName + "_" + e._1.attribute + "," + e._2._1.sourceName + "_" + e._2._1.attribute + "," + e._2._2)
    )*/
    ...
    val edgesPerKey =
      edges.union(
        edges.map {
          case (attr1, (attr2, sim)) =>
            (attr2, (attr1, sim))
        }
      ).groupByKey().map(x => (x._1, x._2.toSet))

    /** For each attribute keeps the attribute with the highest JS, and produce a cluster of elements (k1, k2) */
    val topEdges = edgesPerKey.map {
      case (key1, keys2) =>
        val max = keys2.map(_._2).max * maxFactor
        (key1, keys2.filter(_._2 >= max).map(_._1))
    }
    ...
    if (computeEntropy) {
      ...
      }.toList ::: KeysCluster(defaultClusterID, nonClusteredAttributes.map(k => k.sourceName + separator + k.attribute).toList, defaultEntropy, blob = true) :: Nil
    }
    else {
      clusters.map {
        case (keys, clusterID) =>
          KeysCluster(clusterID, keys.map(k => k.sourceName + separator + k.attribute).toList)
      }.toList ::: KeysCluster(defaultClusterID, nonClusteredAttributes.map(k => k.sourceName + separator + k.attribute).toList, blob = true) :: Nil
    }
  }
  ...
  def createBlocks(profiles: RDD[Profile], numHashes: Int, targetThreshold: Double, numBands: Int = -1, separatorIDs: Array[Long] = Array.emptyLongArray, keysToExclude: Iterable[String] = Nil): RDD[BlockAbstract] = {
    ...
    val sc = SparkContext.getOrCreate()
    val hashes2 = sc.broadcast(profilesPerToken.map {
      case (tokenID, profiles2) =>
        val hashes = getHashes2(tokenID, numHashes)
        (tokenID, hashes)
    }.collectAsMap())

    val tokensPerAttribute = profilesPerToken.flatMap {
      case (tokenID, profiles2) =>
        profiles2.map(p => (p, tokenID))
    }.groupByKey().map(x => (x._1, x._2.toSet))


    val profilesWithSignature: RDD[(Long, Array[Int])] = tokensPerAttribute.map {
      case (profileID, tokens) =>
    ...
    val buckets = profilesWithSignature.map {
      case (attribute, signature) =>
        val buckets = signature.sliding(numRows, numRows).map(_.toList.hashCode()).toIterable
        (attribute, buckets)
    }
